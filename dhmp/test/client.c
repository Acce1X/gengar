...
const double A =
    1.3; //定义参数A>1的浮点数, 后来测试小于1的,似乎也可以,倾斜指数）
const double C =
    1.0; //这个C是不重要的,一般取1, 可以看到下面计算中分子分母可以约掉这个C
...
void pick() {
    ...
    for (i = 0; i < NUM_SIZE; i++) {
        ...
        while (
            index < R - 1 &&
            data >
                pf[index]) //找索引,直到找到一个比他小的值,那么对应的index就是随机数了
            index++;
        rand_num[i] = index;
    }
}
...
int main(int argc, char *argv[]) {
    ...
    if (argc < 4) {
        printf(
            "input param error. input:<filname> <size> <readnum> <writenum>\n");
    ...
    dhmp_client_destroy();
    task_time_diff_ns =
        ((task_time_end.tv_sec * 1000000000) + task_time_end.tv_nsec) -
