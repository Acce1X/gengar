...
struct dhmp_device *dhmp_get_dev_from_watcher() {
    ...
    if (!list_empty(&watcher->dev_list)) {
        res_dev_ptr =
            list_first_entry(&watcher->dev_list, struct dhmp_device, dev_entry);
    }
...
json_object *dhmp_get_json_data() {
    ...
    json_object *dram_total_size_arr[DHMP_SERVER_NODE_NUM],
        *nvm_total_size_arr[DHMP_SERVER_NODE_NUM], *server_jobj, *app_jobj,
        *res, *dram_used_size, *nvm_used_size, *apps_arr;

    for (i = 0; i < watcher->config.nets_cnt; i++) {
        dram_total_size_arr[i] =
            json_object_new_int64(watcher->servers_info[i].dram_total_size);
        nvm_total_size_arr[i] =
            json_object_new_int64(watcher->servers_info[i].nvm_total_size);
    }
    ...
    for (i = 0; i < watcher->config.nets_cnt; i++) {
        ...
        json_object_object_add(server_jobj, "dram total size",
                               dram_total_size_arr[i]);
        json_object_object_add(server_jobj, "nvm total size",
                               nvm_total_size_arr[i]);
        ...
        for (j = 0; j < watcher->cur_app_num; j++) {
            ...
            dram_used_size =
                json_object_new_int64(watcher->apps_info[i][j].dram_used_size);
            nvm_used_size =
                json_object_new_int64(watcher->apps_info[i][j].nvm_used_size);

            json_object_object_add(app_jobj, "app name",
...
void dhmp_watcher_init() {
    ...
    for (i = 0; i < watcher->config.nets_cnt; i++) {
        if (watcher->connect_trans[i] == NULL)
            continue;
        while (watcher->connect_trans[i]->trans_state <
    ...
    pthread_create(&watcher->inform_server_thread, NULL,
                   dhmp_inform_server_thread, (void *)watcher);
}
