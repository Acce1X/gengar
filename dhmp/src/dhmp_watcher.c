...
static json_object *dhmp_get_json_data() {
    int i, j;
...
static void dhmp_inform_tcp_server() {
    json_object *jobj;
    const char *jobj_str;
    int err, size;
...
static int dhmp_build_tcp_connection() {
    ...
    err = connect(watcher->tcp_sockfd, (struct sockaddr *)&server_addr, sizeof(server_addr));
    if (err) {
...
static void *dhmp_inform_server_thread(void *data) {
    uint64_t exp = 0;
...
static void dhmp_fetch_server_info(int node_index, struct dhmp_transport *rdma_trans) {
    struct dhmp_msg msg;
...
struct dhmp_device *dhmp_get_dev_from_watcher() {
    ...
    if (!list_empty(&watcher->dev_list)) {
        res_dev_ptr = list_first_entry(&watcher->dev_list, struct dhmp_device, dev_entry);
    }
