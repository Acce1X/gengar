 ...
 */
static int dhmp_dev_init(struct ibv_context *verbs,
    ...
    dev_ptr->verbs = verbs;
    INFO_LOG("max mr %d max qp mr %d max cqe %d", dev_ptr->device_attr.max_mr,
             dev_ptr->device_attr.max_qp_wr, dev_ptr->device_attr.max_cqe);
    return retval;
...
void dhmp_dev_list_init(struct list_head *dev_list_ptr) {
    ...
    for (i = 0; i < num_devices; i++) {
        ...
        if (!dev_ptr->verbs) {
            ERROR_LOG("RDMA device [%d]: name= %s allocate error.", i,
                      ibv_get_device_name(ctx_list[i]->device));
        } else {
            INFO_LOG("RDMA device [%d]: name= %s allocate success.", i,
                     ibv_get_device_name(ctx_list[i]->device));
...
void dhmp_dev_list_destroy(struct list_head *dev_list_ptr) {
    ...
    list_for_each_entry_safe(dev_tmp_ptr, dev_next_tmp_ptr, dev_list_ptr,
                             dev_entry) {
