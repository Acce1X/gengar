...
void dhmp_handle_cache_model(int node_index, int length) {
    ...
    while (start < end) {
        ...
        if (start < end) {
            cold_nvm[node_index][cold_cnt[node_index]].nvm_addr =
                sa_entries[node_index][start].nvm_addr;
            cold_nvm[node_index][cold_cnt[node_index]].length =
                sa_entries[node_index][start].length;
            ++cold_cnt[node_index];

            hot_nvm[node_index][hot_cnt[node_index]].nvm_addr =
                sa_entries[node_index][end].nvm_addr;
            hot_nvm[node_index][hot_cnt[node_index]].length =
                sa_entries[node_index][end].length;
            client->req_dram_size[node_index] +=
                hot_nvm[node_index][hot_cnt[node_index]].length;
            ++hot_cnt[node_index];
    ...
    --start;
    while (start >= 0 && sa_entries[node_index][start].rwcnt >=
                             client->threshold[node_index]) {
...
void dhmp_poll_ht_func(void) {
    ...
    double per_benefit = 0.0, per_benefit_det = 0.0, cur_hit_ratio = 0.0,
           hit_ratio_det = 0.0;
    ...
    for (i = 0; i < DHMP_SERVER_NODE_NUM; i++) {
        ...
        /*set the diff between the request dram size and the response dram
         * size*/
    ...
    for (i = 0; i < DHMP_CLIENT_HT_SIZE; i++) {
        ...
        hlist_for_each_entry(addr_info, &client->addr_info_ht[i], addr_entry) {
            ...
            client->access_region_size +=
                rw_cnt * (addr_info->nvm_mr.length / 1024);
            ...
            total_benefit_denominator[node_index] +=
                addr_info->read_cnt *
                (DHMP_RTT_TIME +
                 DHMP_DRAM_RW_TIME * addr_info->nvm_mr.length / 1024 +
            ...
            if (addr_info->dram_mr.addr != NULL) {
                ...
                total_benefit_numerator[node_index] +=
                    addr_info->read_cnt *
                    ((addr_info->nvm_mr.length / PAGE_SIZE + 1) * rdelay /
                     knum);

                total_benefit_numerator[node_index] +=
                    addr_info->write_cnt *
                    ((addr_info->nvm_mr.length / PAGE_SIZE + 1) * wdelay /
                     knum);
            ...
            if (!client->dram_threshold_policy[node_index]) {
                if ((rw_cnt >= client->threshold[node_index]) ||
                    (rw_cnt < client->threshold[node_index] &&
                     addr_info->dram_mr.addr != NULL)) {
                    sa_entries[node_index][sa_index[node_index]].nvm_addr =
                        addr_info->nvm_mr.addr;
                    sa_entries[node_index][sa_index[node_index]].length =
                        addr_info->nvm_mr.length;
                    sa_entries[node_index][sa_index[node_index]].rwcnt = rw_cnt;

                    if (addr_info->dram_mr.addr == NULL)
                        sa_entries[node_index][sa_index[node_index]].in_dram =
                            false;
                    else
                        sa_entries[node_index][sa_index[node_index]].in_dram =
                            true;
    ...
    for (i = 0; i < DHMP_SERVER_NODE_NUM; i++) {
        if (client->dram_threshold_policy[i] == false) {
            qsort(sa_entries[i], sa_index[i],
                  sizeof(struct dhmp_sort_addr_entry), comp_sort_addr_entry);
    ...
    for (i = 0; i < DHMP_SERVER_NODE_NUM; i++) {
        if (client->dram_threshold_policy[i]) { /*pool*/
            ...
            if (client->access_total_num != 0) {
                if (client->access_dram_num[i] == 0)
                    ...
                    per_benefit =
                        (double)(((long double)total_benefit_numerator[i] /
                                  total_benefit_denominator[i]) /
                                 dram_cache_size[i]);
                DEBUG_LOG("pre perbenefit %lf cur perbenefit %lf",
                          client->per_benefit[i], per_benefit);
                if (client->per_benefit[i] != 0.0 && per_benefit != 0.0)
                    per_benefit_det =
                        (per_benefit - client->per_benefit[i]) / per_benefit;
                else if (client->per_benefit[i] == 0.0 && per_benefit != 0.0)
                    per_benefit_det =
                        (per_benefit - client->per_benefit_max[i]) /
                        per_benefit;
                ...
                if (per_benefit_det > 0.0 && per_benefit_det < 1.0)
                    client->threshold[i] =
                        (int)(client->threshold[i] * (1 - per_benefit_det) +
                              0.5);
            ...
            if (client->access_total_num != 0) {
                if (client->pre_hit_ratio[i] < 0.0) {
                    cur_hit_ratio = ((double)client->access_dram_num[i] /
                                     client->access_total_num);
                    ...
                    if (client->pre_hit_ratio[i] != 0.0 && cur_hit_ratio != 0.0)
                        hit_ratio_det =
                            (cur_hit_ratio - client->pre_hit_ratio[i]) /
                            cur_hit_ratio;
                    else if (client->pre_hit_ratio[i] == 0.0 &&
                             cur_hit_ratio != 0.0)
                        hit_ratio_det =
                            (cur_hit_ratio - client->pre_hit_ratio_max[i]) /
                            cur_hit_ratio;
                    ...
                    if (hit_ratio_det > 0.0 && hit_ratio_det < 1.0)
                        client->threshold[i] =
                            (int)(client->threshold[i] * (1 - hit_ratio_det) +
                                  0.5);
    ...
    if (client->access_total_num == 0)
        ...
        cur_average_size = (size_t)(client->access_region_size /
                                        (double)client->access_total_num +
                                    0.5);
        if (client->pre_average_size == 0) {
            total_times = DHMP_DEFAULT_POLL_TIME;
            tmp_total_times =
                (long long)(total_times * ((long double)cur_average_size /
                                           DHMP_DEFAULT_SIZE));
        } else {
    ...
    INFO_LOG("client sec %ld,nsec %ld", client->poll_interval.tv_sec,
             client->poll_interval.tv_nsec);
