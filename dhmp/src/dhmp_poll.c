...
struct dhmp_nvm_info hot_nvm[DHMP_MAX_SERVER_GROUP_NUM][DHMP_MAX_OBJ_NUM],
    cold_nvm[DHMP_MAX_SERVER_GROUP_NUM][DHMP_MAX_OBJ_NUM];
int hot_cnt[DHMP_MAX_SERVER_GROUP_NUM], cold_cnt[DHMP_MAX_SERVER_GROUP_NUM];
struct dhmp_sort_addr_entry sa_entries[DHMP_MAX_SERVER_GROUP_NUM][DHMP_MAX_OBJ_NUM];
int max_cnt_test = 0;
...
void dhmp_poll_ht_func(void) {
    ...
    long long total_benefit_numerator[DHMP_MAX_SERVER_GROUP_NUM], total_benefit_denominator[DHMP_MAX_SERVER_GROUP_NUM];

    size_t dram_size_diff[DHMP_MAX_SERVER_GROUP_NUM], dram_cache_size[DHMP_MAX_SERVER_GROUP_NUM];
    int sa_index[DHMP_MAX_SERVER_GROUP_NUM];
        ...
        if (client->dram_threshold_policy[i]) { /*pool*/
            ...
            if (client->access_total_num != 0) {
                if (client->access_dram_num[i] == 0)
                    ...
                    per_benefit = (double)(((long double)total_benefit_numerator[i] / total_benefit_denominator[i]) /
                                           dram_cache_size[i]);
    ...
    INFO_LOG("client sec %ld,nsec %ld", client->poll_interval.tv_sec, client->poll_interval.tv_nsec);
